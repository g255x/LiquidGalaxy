...
<div class="setting">
    <div class="name">{cSetting.translatedName ?? (spaceSeperatedNames ? convertToSpacedString(cSetting.name) : cSetting.name)}</div>

    <div class="body">
        <button class="button-select" on:click={selectFile}>{cSetting.value === "" ? "<空>" : cSetting.value}</button>
        ...
        {#if cSetting.value !== ""}
            ...
            <button class="button-action" on:click={() => browsePath(cSetting.value)}>
                <img class="icon" src="img/clickgui/icon-open-file.svg" alt="open-file" title="打开" />
            </button>
        {/if}
    </div>
</div>
